...
# Chargement de configuration JSON
...
if st.sidebar.button(
  ...
  with st.spinner(
    ...
    # Utilisation de Differential Evolution pour s'affranchir de la valeur initiale
    res = differential_evolution(
        ...
        maxiter=200,
        popsize=15,
        polish=True,  # Affinage final automatique par L-BFGS-B
...
# ==============================================================================
# 4. OPTIMISATION AUTOMATIQUE DES PARAMÈTRES
# ==============================================================================
st.sidebar.markdown("---")
st.sidebar.subheader("🎯 Calibrage Automatique")
...
if st.sidebar.button(
    "🤖 Ajuster les paramètres au dataset",
    help=(
        "❓ Lance l'optimisation pour estimer les paramètres minimisant l'erreur"
        " quadratique."
    ),
):
  lnT_vec = df["lnT"].to_numpy()
  ...
  with st.spinner("Optimisation des paramètres en cours..."):
    initial_params = [A, B, C1, omega, phi1, C2, phi2]
    res = minimize(
        loss_func_fast, initial_params, bounds=bounds, method="L-BFGS-B"
    )
  ...
  if res.success:
    ...
    st.sidebar.error("L'optimisation a échoué.")
